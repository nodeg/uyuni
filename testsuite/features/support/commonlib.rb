...
#
# The timeout is only checked between iterations, so a single yield call that blocks indefinitely
...
def repeat_until_timeout(timeout: DEFAULT_TIMEOUT, retries: nil, message: nil, report_result: false, dont_raise: false)
  ...
  # Monotonic clock avoids NTP syncs/system time changes breaking the timeout
  ...
  begin
    loop do
      ...
      if retries && attempts >= retries
        ...
        raise ScriptError, "Giving up after #{attempts} attempts#{detail}"
      end

      elapsed_time = Process.clock_gettime(Process::CLOCK_MONOTONIC) - start_time
      next if elapsed_time < timeout

      detail = format_detail(message, last_result, report_result)
      $stdout.puts "Timeout after #{timeout} seconds#{detail}"
