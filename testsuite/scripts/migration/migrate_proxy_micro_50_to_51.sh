...
#
# Complete proxy migration on SL Micro after Web UI product migration
...
verify_upgrade() {
    ...
    ssh root@"${HOSTNAME}" "cat /etc/os-release | grep 'SUSE Linux Micro 6.1'"
    ssh root@"${HOSTNAME}" "SUSEConnect --status-text | grep 'SUSE Multi-Linux Manager Proxy Extension 5.1'"
    ssh root@"${HOSTNAME}" "mgrpxy --version | grep '5.1'"
}

install_new_images() {
    echo "Installing new proxy images..."
    ssh root@"${HOSTNAME}" "transactional-update pkg install suse-multi-linux-manager-5.1-x86_64-proxy*"
}

reboot_after_install() {
    echo "Rebooting after package install..."
    ssh root@"${HOSTNAME}" "reboot" || true
    sleep 60
}
...
main() {
    echo "==> Completing proxy migration on SL Micro"
