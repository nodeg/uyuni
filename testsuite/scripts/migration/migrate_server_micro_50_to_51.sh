...
#
# Migrate MLM 5.0 server to 5.1 on SL Micro 5.5 → 6.1
...
apply_patches() {
    ...
    ssh root@"${HOSTNAME}" "transactional-update patch"
}
...
migrate_os() {
    echo "Migrating OS to SL Micro 6.1..."
    ssh root@"${HOSTNAME}" "transactional-update migration --auto-agree-with-licenses --gpg-auto-import-keys"
}
...
main() {
    echo "==> Migrating MLM 5.0 server to 5.1 on SL Micro"
